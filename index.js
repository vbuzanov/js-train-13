...
function complexConvert(data) {
  ...
  for (const key in objParse) {
    if (typeof objParse[key] === "number") {
      obj[key] = ++objParse[key];
    } else {
      obj[key] = objParse[key];
    }
  }
...
function searchParamsURL(url) {
  ...
  let map = new Map();
  param.forEach((value, key) => map.set(key, value));
  return map;
}
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  const keysParamsObj = Object.keys(paramsObj);
  for (const key of keysParamsObj) {
...
function checkURLParameters(url, params) {
  ...
  for (const item of params) {
    objResult[item] = urlObj.searchParams.has(item);
  }
  return objResult;
}
...
function sortUrlParams(url) {
  ...
  // Повертаємо новий URL як рядок.
  let urlObj = new URL(url);
  const arr = [...urlObj.searchParams.entries()];
  arr.sort();
  arr.forEach((value) => urlObj.searchParams.delete(value[0]));
  arr.forEach((value) => urlObj.searchParams.append(value[0], value[1]));
  return urlObj.toString();
}
...
function getURLValues(url) {
  ...
  const keysParam = [...param.keys()];
  let valuesParam = [];
  keysParam.forEach((value) => {
    valuesParam.push(...param.getAll(value));
  });
  return valuesParam;
}
...
function getUrlKeys(url) {
  ...
  let urlObj = new URL(url);
  const keysParam = [...urlObj.searchParams.keys()];
  return keysParam;
}
